                    ...
                    item={item => {
                      return (
                        <Card  full  className='scrollList-item' key={item.id} onClick={()=>this.toDetail(item.id)}>
                          <Card.Header
                            title={<span style={{ fontSize: 15,color: '#333',fontWeight: 'bold' }}>上架单号：{item.id}</span>}
                            extra={<span >零库存补货  待上架</span>}
                          />
                          <Card.Body>
                            <Flex style={{ color: '#555' }}>
                              <Flex.Item>验收时间 :<span>{item.date }</span></Flex.Item>
